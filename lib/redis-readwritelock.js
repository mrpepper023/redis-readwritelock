...
(function() {
  ...
  randomwait = function(waittimeobj, next) {
    ...
    if (usewaittime < 1) {
      ...
      return setTimeout(next, usewaittime);
    }
  };
  ...
  isconflictrangelock = function(targetobj, rangemin, rangemax, next) {
    if (next == null) {
      if (lockconfig.log) {
        console.log('no next');
      }
    ...
    return rcli.INCR([objectname + ':uniquename'], function(err, reply) {
      ...
      return cs._while([], function(_break, _next) {
        return readerlock(lockobj.obj, function(metalockobj) {
          return isconflictrangelock(lockobj.wobj, rangemin, rangemax, function(isconflict) {
            var args;
            if (!isconflict) {
    ...
    return rcli.INCR([objectname + ':uniquename'], function(err, reply) {
      ...
      return cs._while([], function(_break, _next) {
        return pwrlock(lockobj.obj, function(metalockobj) {
          return isconflictrangelock(lockobj.pwrobj, lockobj.min, lockobj.max, function(isconflict) {
            if (!isconflict) {
              return isconflictrangelock(lockobj.wobj, lockobj.min, lockobj.max, function(isconflict) {
                if (!isconflict) {
              ...
              return rcli.ZADD(args, function(err, reply) {
                return unlock(metalockobj, function() {
                  return unlock(metalockobj, function() {
                    waittimeobj.time = lockconfig.waitmin;
                    return cs._while([], function(_break, _next) {
                      return isconflictrangelock(lockobj.robj, lockobj.min, lockobj.max, function(isconflict) {
                        if (!isconflict) {
                          return _break();
                        } else {
                          return randomwait(waittimeobj, _next);
